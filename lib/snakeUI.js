(function() {
  ...
  SnakeUI.prototype.pauseGame = function() {
    $('#' + this.displayId).siblings('#pause-screen').toggleClass('hide');
  ...
  SnakeUI.prototype.displayScore = function() {
    $('#' + this.displayId).siblings().find('#score').html(this.board.score);
  };
  ...
  SnakeUI.prototype._bindButtons = function() {
    ...
    $('#pause-screen').on('click', 'button', function(event) {
      switch(this.id) {
        case 'resume-button':
          that.pauseGame();
          break;
